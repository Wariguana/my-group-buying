...
const transaction = vi.hoisted(() => ({
  groupBuy: { findMany: vi.fn(), findUnique: vi.fn(), create: vi.fn(), update: vi.fn() },
...
beforeEach(() => {
  ...
  transaction.groupBuy.update.mockResolvedValue({ id: groupBuyId });
...
test.each(["PUBLISHED", "CANCELLED"] as const)("refuses to edit %s without writes", async (status) => {
  ...
  expect(transaction.groupBuy.update).not.toHaveBeenCalled();
});
...
test("retains inactive existing references, preserves item cost, updates allowed values, and does not query master data", async () => {
  ...
  expect(transaction.groupBuy.update.mock.calls[0][0].data).not.toHaveProperty("slug");
  expect(transaction.groupBuy.update.mock.calls[0][0].data).not.toHaveProperty("status");
  expect(transaction.groupBuy.update.mock.calls[0][0].data).not.toHaveProperty("publishedAt");
});
...
test("service has no GroupBuy hard delete, publish flow, or Product/PickupLocation mutation", async () => {
  ...
  expect(source).not.toMatch(/publishGroupBuy|status:\s*"PUBLISHED"/);

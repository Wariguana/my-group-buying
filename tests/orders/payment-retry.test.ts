...
import {
  PaymentClaimLostError,
  MAX_PAYMENT_CONCURRENCY_RETRIES,
  retryPaymentTransaction,
} from "@/lib/orders/payment-retry";
...
test.each([
  ...
  ["claim lost", new PaymentClaimLostError()],
])("retries the complete transaction for %s", async (_label, conflict) => {
  const runAttempt = vi.fn()
    .mockRejectedValueOnce(conflict)
    .mockResolvedValueOnce("cancelled");
  const sleep = vi.fn(async () => undefined);
  await expect(retryPaymentTransaction(runAttempt, {
    random: () => 0.5,
    sleep,
...
test.each(["ACCESS_DENIED", "CANCELLED"] as const)(
  ...
  async (code) => {
    const failure = new PaymentOrderError(code);
    const runAttempt = vi.fn().mockRejectedValue(failure);
    await expect(retryPaymentTransaction(runAttempt)).rejects.toBe(failure);
...
test("unexpected failure is sanitized without retry", async () => {
  ...
  const result = await retryPaymentTransaction(runAttempt).catch((error: unknown) => error);
  expect(result).toMatchObject({ code: "FAILED", message: "The payment failed." });
...
test("bounds whole-transaction conflict retries", async () => {
  ...
  expect(runAttempt).toHaveBeenCalledTimes(1 + MAX_PAYMENT_CONCURRENCY_RETRIES);
});

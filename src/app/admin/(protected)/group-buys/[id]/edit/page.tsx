...
export default async function EditGroupBuyPage({ params }: PageProps<"/admin/group-buys/[id]/edit">) {
  ...
  return <section><PageHeader eyebrow={isPublished ? "已發布" : "草稿"} title={isPublished ? "編輯已發布團購" : "編輯團購草稿"} description={result.value.title} />{hasOrders && <div className="mt-6 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950">此團購已有訂單。圖片調整只影響頁面呈現；既有訂單的歷史價格、商品與取貨資料不會被改寫。</div>}<GroupBuyForm action={updateGroupBuyDraftAction.bind(null, result.value.id)} submitLabel={isPublished ? "儲存修改" : "儲存草稿"} productOptions={products.value} pickupOptions={pickups.value} stockLocked={hasOrders} values={{ title: result.value.title, description: result.value.description, gallery: result.value.images.map((image) => ({ kind: "existing" as const, id: image.id, imageUrl: image.imageUrl })), startAt: formatTaipeiDateTimeLocal(result.value.startAt), endAt: formatTaipeiDateTimeLocal(result.value.endAt), allowsSelfPickup: result.value.allowsSelfPickup, allowsSevenEleven: result.value.allowsSevenEleven, items: result.value.items.map((item) => ({ productId: item.productId, salePrice: String(item.salePrice), stock: item.stock == null ? "" : String(item.stock), purchaseLimit: item.purchaseLimit == null ? "" : String(item.purchaseLimit) })), pickups: result.value.pickups.map((pickup) => ({ pickupLocationId: pickup.pickupLocationId, pickupStartAt: pickup.pickupStartAt ? formatTaipeiDateTimeLocal(pickup.pickupStartAt) : "", pickupEndAt: pickup.pickupEndAt ? formatTaipeiDateTimeLocal(pickup.pickupEndAt) : "" })) }} />{!isPublished && <PublishGroupBuyForm action={publishGroupBuyAction.bind(null, result.value.id)} />}</section>;
}
